...
var navigationRight = jQuery('.menus-wrap');
...
function Navigation() {
    ...
    function toggleMenu() {
        if (isOpen) {
            classie.remove(bodyEl, 'show-menus');
        } else {
            classie.add(bodyEl, 'show-menus');
        }
        isOpen = !isOpen;
    }
...
jQuery(document).ready(function ($) {
    ...
    jQuery('.menus-item-has-children, .page_item_has_children').hover(function () {
        jQuery(this).children('.sub-menus').stop().slideDown(200);
    },
        function () {
            jQuery(this).children('.sub-menus').stop().slideUp(200);
        });

    //Mobile Menu Open/Close 
    jQuery('#open-mobile-menus').on('click', function () {
        var self = jQuery(this);
        var mobileMenu = jQuery('.menus-wrap-2');
